...
class BatteryAlarm(StdService):
    ...
    def __init__(self, engine, config_dict):
        ...
        try:
            ...
            log.info(
                "LowBattery alarm enabled. Count threshold is %d", self.count_threshold
            )
    ...
    def new_loop_packet(self, event):
        ...
        low_batteries = {
            k: packet[k]
            for k in BatteryAlarm.battery_flags
        ...
        if low_batteries:
            ...
            if self.alarm_count >= self.count_threshold:
                ...
                if abs(time.time() - self.last_msg_ts) >= self.time_wait:
                    ...
                    # main LOOP thread:
                    t = threading.Thread(
                        target=BatteryAlarm.sound_the_alarm,
                        args=(self, timestamp, low_batteries, self.alarm_count),
                    )
                    t.start()
    ...
    def sound_the_alarm(self, timestamp, battery_flags, alarm_count):
        ...
        msg_text = """The low battery indicator has been seen %d times since the last archive period.
            Alarm sounded at %s 
            Low battery indicators: %s
""" % (
            alarm_count,
            t_str,
        ...
        try:
            ...
            s = smtplib.SMTP_SSL(self.smtp_host, 587, local_hostname="blaik.my.to")
            s.starttls()
            log.debug("Using SMTP_SSL")
        except (AttributeError, ssl.SSLError):
            # If that doesn't work, try creating an insecure host, then upgrading
            s = smtplib.SMTP(self.smtp_host, 587, local_hostname="blaik.my.to")
            try:
